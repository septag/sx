...
// You create a fiber context, which is actually a fiber pool. On each frame of the game you update
// the context So when you invoke a fiber, it will run it like a normal function,
// but you can return in the middle of the function and continue on some other time based on
// 'sx_fiber_ret_type' enum value.
...
//      sx_fiber_update             Updates fiber-context state with a delta-time as input.
//                                  In the game this should be called on each frame
//      sx_fiber_return             Exits the fiber execution and returns to program,
//                                  much like C's 'return' keyword,
//                                  This function must be called whenever you want to exit the fiber
//                                  callback, even at the end of the function
//                                  'pfrom' is the caller fiber and gets updated, should be the
//                                      pointer to the sx_fiber_t handle 'transfer.from' in the
//                                      parameter of the callback
//                                  'sx_fiber_ret_type' indicates how should the fiber continue it's
//                                      execution on the next updates
//                                      SX_FIBER_RET_FINISH indicates that execution is finished and
//                                                          fiber's work is done.
//                                                          This can be called in the middle of
//                                                          fiber callback
//                                                          And must always be called at the end of
//                                                          the function body
//                                      SX_FIBER_RET_PASS   indices that execution must continue on
//                                                          next N frames given by 'arg' parameter
//                                      SX_FIBER_RET_WAIT   waits for N msecs and continue the
//                                                          execution, which is 'arg' paramater
//
#pragma once
...
typedef enum sx_fiber_ret_type {
    ...
    SX_FIBER_RET_PASS,      // Pass this 'update' to the next N update which is 'arg' in
                            // sx_fiber_return
