...
#define _sx_sbgrow(_alloc,a,n)      (*((void **)&(a)) = sx__sbgrowf((a), (n), sizeof(*(a)), (_alloc), __FILE__, __LINE__))
                            ...
                            const char* file, int line)
{
   ...
   int* p = (int*)sx__realloc(alloc, arr ? _sx_sbraw(arr) : 0, itemsize*m + sizeof(int)*2, 0, file, line);
   if (p) {
        if (!arr)
            p[1] = 0;
        p[0] = m;
