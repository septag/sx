...
void sx_hashtbl_remove(sx_hashtbl* tbl, int index);
bool sx_hashtbl_full(const sx_hashtbl* tbl);
int  sx_hashtbl_get(const sx_hashtbl* tbl, int index);
int  sx_hashtbl_find(const sx_hashtbl* tbl, uint32_t key);
int  sx_hashtbl_find_get(const sx_hashtbl* tbl, uint32_t key, int not_found_val SX_DFLT(-1));
